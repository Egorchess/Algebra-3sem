...
\[\forall g \in G: \alpha(g^{-1}) = (\alpha(g))^{-1} \Longrightarrow \alpha(g^{-1})x = (\alpha(g))^{-1}(x)\]
...
\begin{proof}
    ...
    \[g_1 \textup{St}(x) = g_2 \textup{St}(x) \Longleftrightarrow \{h \in G \ | \ g_1x = hx\} = \{h \in G \ | \ g_2x = hx\} \Longleftrightarrow g_1x = g_2x\]
    \begin{itemize}
        \item Корректность: $g_1x = g_2x \Longrightarrow g_1 \textup{St}(x) = g_2 \textup{St}(x)$;
        \item Сюръективность --- очевидна ($\forall g \in G$ $g\textup{St}(x)$ будет образом точки $gx$);
        \item Инъективность: $g_1 \textup{St}(x) = g_2 \textup{St}(x) \Longrightarrow g_1x = g_2x$.
    \end{itemize}
    Отсюда $\psi$ --- биекция.
\end{proof}
...
\end{proof}
\begin{definition}
    $\\ \textup{Aut }G$ называется группой \st{аутизмов}$_{\href{https://ru.wikipedia.org/wiki/\%D0\%90\%D1\%83\%D1\%82\%D0\%B8\%D0\%B7\%D0\%BC}{1}}$ автоморфизмов группы $G$.\\
...
\end{subtheoremnum}
\begin{proof}
    $\forall g \in G: gxg^{-1} = x \Longleftrightarrow gx = xg$.
\end{proof}
\begin{definition}
