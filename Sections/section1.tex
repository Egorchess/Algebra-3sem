...
\end{proof}
\begin{subtheorem}
    Отношение $a\equiv b \pmod{H} \Leftrightarrow b^{-1}a\in H$ является отношением эквивалентности, причём классы эквивалентности совпвдвют с левыми смежными классами (аналогично $ab^{-1} \in H$ для правых).
\end{subtheorem}
\begin{proof}\tab
    \begin{itemize}
...
\end{example}
\begin{consequensenum}
    Любая группа $G$ простого порядка $p$ явзяется циклической.
\end{consequensenum}
\begin{proof}
...
\end{definition} 
\begin{proof}[Эквивалентость определений:]\tab
    \begin{itemize}
