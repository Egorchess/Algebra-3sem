...
\begin{examples} \tab
    ...
    \end{enumerate}
\end{examples}
\begin{subtheorem}[Свойства элементов конечного порядка]\tab
    \begin{enumerate}
...
\begin{proof}
    ...
    Если $\textup{ord } g = n: \langle g \rangle = \{e, g^1, ... g^{n-1}\}$ - все эти элементы различны из пункта 2 теоремы, а других нет по определению порядка.
\end{proof}
\begin{examples} \tab
    \begin{enumerate}
...
\begin{examples}\tab
    \begin{enumerate}
        ...
        $U_n$ образует группу относительно умножения, причём $U_n = \langle \cos \frac{2\pi}{n} + i \sin \frac{2\pi}{n} \rangle$.
    \end{enumerate}    
\end{examples}
\begin{subtheorem}
...
\end{subtheorem}
\begin{proof}
    Очевидно из определения порожающего множества.
\end{proof}
\begin{remark}
    Далее циклическую группу порядка $n$ обозначаем $\langle g \rangle_n$
\end{remark}
\begin{subtheorem}
    Пусть $G = \langle g \rangle_n$. Тогда $G = \langle g^k \rangle \Longleftrightarrow \text{НОД}(k, n) = 1$.
\end{subtheorem}
\begin{proof}
    
\end{proof}
\begin{theorem}[Классификация циклических групп] \tab
    \begin{enumerate}
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        $k \equiv m (\mod n) \Longleftrightarrow g^k = g^n$ - отсюда $\phi$ - инъекция.\\
        Сохранение операции - аналогично пункту 1.\\ 
        Отсюда $\phi$ - изоморфизм.
    \end{enumerate}
\end{proof}
\begin{consequense}
    Пусть $G_1, G_2$ - циклические группы. Тогда $G_1 \cong G_2 \Longleftrightarrow |G_1| = |G_2|$.
\end{consequense}
\begin{proof}\tab
...
\begin{theorem} \tab
    \begin{enumerate}
        ...
        \item Подгруппы циклической группы $G$ порядка $n$ находятся во взаимном соответствии с делителями $n$, т.е. $\forall H \leqslant G \ |H| | n$ и $\forall d | n \ \exists! \ H \leqslant G : |H| = d$.
...
\begin{proof} \tab
    \begin{enumerate}
        ...
        Так как $g^n = e \in H$, то $k | n \Longrightarrow n = kq$. Отсюда $H = \{e, g^k, g^{2k},...,g^{(q-1)k}\} \Longrightarrow |H| = q | n$.\\
