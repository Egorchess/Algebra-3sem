...
\end{definition}
\begin{definition}
    Поле --- коммутативное ассоциативное кольцо с единицей $1 \neq 0$, в котором любой ненулевой элемент обратим.
\end{definition}
\begin{examples}\tab
    \begin{enumerate}
...
\begin{proof}\tab
    \begin{enumerate}
        \item Сложение корректно из корректности сложения смежных классов в группах, так как $(I, +)$ --- подгруппа $(K, +)$, нормальная из абелевости;
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        \item Пусть $I$ --- двусторонний идеал кольца $K$ такой, что $S \subset I$. Тогда из определения идеала $\forall a \in K, s \in S: \ as \in I$, а так как идеал --- подкольцо, любая сумма вида $\sum \limits_{i=0}^ka_is_i$, где $a_i \in K, s_i \in S$, лежит в $I$. Значит, $(S) \subseteq I$.
    \end{enumerate}
\end{proof}
\begin{definition}
...
\end{examples}
\begin{remark}
    Все напоминания данной главы взяты из курса 1 семестра. %\cite{algebra}.
\end{remark}
\begin{reminder}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\forall a, b \in K: N(ab) \geqslant N(a)$;
...
\begin{examples}\tab
    \begin{enumerate}
        ...
        \begin{itemize}
            \item $f \sim 1 \Longrightarrow 1 = 2a + xb$ --- невозможно, т.к. коэффициенты целые;
            \item $f \sim 2 \Longrightarrow x = 2 \cdot q_2$ --- невозможно по тем же соображениям.
        \end{itemize}
...
\begin{proof}
    ...
    Докажем, что $\E = \{e_ig_j \ | \ i = \overline{1,n}, j = \overline{1,m}\}$ --- базис $M$ как векторного пространства над $F$:
    \begin{enumerate}
...
\begin{proof} $F[u] = \{g(u) \ | \ g \in F[x]\}$
    \begin{enumerate}
        ...
        Пусть $x \in F[a] \Longrightarrow x = \sum \limits_{j=0}^m \lambda_ja^j$. Индукция по $m$:\\
...
\begin{definition}
    Пусть $L$ --- расширение поля $F$, $M = \{a_i \ | \ i \in I\} \subset L$. Подполем, порождённым элементами из $M$ над $F$, называется наименьшее подполе $L$, содержащее $M$ и $F$. Обозначается $F(M)$.
\end{definition}
\begin{subtheorem}
    \[F(M) = \left\{\frac{f(a_{i_1},...,a_{i_m})}{g(a_{i_1},...,a_{i_m})}\left|\right. m \in \N, i_k \in I, \ f, g \in F[x_1,...,x_m], \ g(a_1,...,a_n) \neq 0\right\}\]
\end{subtheorem}
\begin{proof}
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item $1 \in \overline{F}, \ a^{-1} \in \overline{F} (a \neq 0) \Longrightarrow \overline{F}$ --- подполе.
        \end{itemize}
...
\begin{proof}
    ...
    Для многочленов знаем, что $f_i \mid f$ над $L_i \Longrightarrow \phi_i(f_i) \mid \phi_i(f)$ над $\tilde{L}$, а $\phi_i(f) = f$, так как все $\phi_i$ тождественны на $F$. При этом $f$ раскладывается на линейные множители над $\tilde{L}$ --- значит, у $\phi_i(f_i)$ не может не быть корней над $\tilde{L}$, и расширение возможно.\\
