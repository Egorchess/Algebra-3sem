...
\end{subtheorem}
\begin{proof}
    \[\forall g \in G, g' = [x_1, y_1][x_2, y_2]...[x_k, y_k] \in G': gg'g^{-1} = g[x_1, y_1][x_2, y_2]...[x_k, y_k]g^{-1} =\]\[(g[x_1, y_1]g^{-1})(g[x_2, y_2]g^{-1})...(g[x_k, y_k]g^{-1}) = [gx_1g^{-1}, gy_1g^{-1}]...[gx_kg^{-1}, gy_kg^{-1}]\]
    Отсюда $\forall g \in G, g' \in G': gg'g^{-1} \in G' \Longrightarrow G' \unlhd G$.
\end{proof}
\begin{subtheorem}
...
\end{subtheorem}
\begin{proof}
    $\\\Longrightarrow: \ $ Пусть $G/N$ абелева. Тогда $\forall g_1, g_2 \in G: \ (g_1N)(g_2N) = (g_2N)(g_1N) \Longrightarrow g_1g_2N = g_2g_1N \Longrightarrow g_1g_2g_1^{-1}g_2^{-1} = [g_1, g_2] \in N$. Значит, любой коммутатор $\in N$, а значит и все произведения коммутаторов $\in N$, то есть $G' \subseteq N$.
...
\begin{lemmanum}\tab
    \begin{enumerate}
        \item $A_n$ порождается циклами длины 3 ($n \geqslant 3$);
        \item Если $n \geqslant 5$, то $A_n$ порождается произведениями пар независимых транспозиций;
    \end{enumerate}    
\end{lemmanum}
\begin{proof}
    $\forall \sigma \in A_n \ \sigma = \prod \limits_{i=1}^k \tau_i$, где $\tau_i$ --- транспозиции, $k$ --- чётное, т.е. транспозиции разбиваются на пары --- рассмотрим случаи зависимых и независимых транспозиций в паре:\\
...
\begin{proof}
    ...
    Если $a_{11} \neq 1$, то сделаем $a_{21} \neq 0$ аналогично $a_{11}$, а далее прибавим к первой строке вторую, умноженную на $\frac{1-a_{11}}{a_{21}}$ --- сделаем $a_{11} = 1$.
    Далее с помощью первой строки сможем занулить оставшиеся элементы первого столбца. Тогда подматрица полученной матрицы без первой строки и первого столбца будет иметь определитель, равный определителю всей матрицы, то есть 1, а отсюда её можно привести к единичной по предположению индукции. Сделаем это, а далее с помощью $i$-й строки занулим $a_{1i}$.\\
...
\begin{proof}
    ...
    Любое ур-е $(\lambda^2 - 1)a = \mu$ решается для $a$, так как $\lambda \neq \pm 1$ --- отсюда все верхнетреугольные элементарные матрицы $I$ типа принадлежат $GL_n(\F)' $. Аналогично для нижнетреугольных --- все элементарные матрицы $I$ типа, а отсюда и $SL_n(\F)$, принадлежат $GL_n(\F)'$.\\
