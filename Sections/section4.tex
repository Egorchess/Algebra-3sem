...
\begin{proof}
    ...
    Рассмотрим $\tilde{B} = B \cap \tilde{A}$ --- подгруппу $B$, которая содержится в $\tilde{A}$ (очевидно, что это подгруппа). По предположению индусов $\tilde{B}$ --- свободная абелева, причём $\textup{rk }\tilde{B} \leqslant \textup{rk }\tilde{A} = n-1$.\\
