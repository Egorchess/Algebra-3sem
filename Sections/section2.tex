...
\end{example} 
\begin{definition}
    Слово $X = x_1...x_k$ --- сократимое, если $\exists i \in \{1,...,k-1\}: x_i = x^{-1}_{i+1}$.\\
...
\begin{proof}
    ...
    \[\phi(W\tilde{W}) = \phi(a_{i_1}^{\epsilon_1}...a_{i_k}^{\epsilon_k}\cdot a_{j_1}^{\tau_1}...a_{j_m}^{\tau_m}) = \phi(a_{i_1}^{\epsilon_1}...a_{i_{k-s}}^{\epsilon_{k-s}}a_{j_{s+1}}^{\tau_{s+1}}...a_{j_m}^{\tau_m}) = g_{i_1}^{\epsilon_1}...g_{i_{k-s}}^{\epsilon_{k-s}}g_{j_{s+1}}^{\tau_{s+1}}...g_{j_m}^{\tau_m} \]
    \[= g_{i_1}^{\epsilon_1}...g_{i_k}^{\epsilon_k}\cdot g_{j_1}^{\tau_1}...g_{j_m}^{\tau_m} =
    (g_{i_1}^{\epsilon_1}...g_{i_k}^{\epsilon_k})\cdot (g_{j_1}^{\tau_1}...g_{j_m}^{\tau_m}) = \phi(W)\phi(\tilde{W})\]
    (где $a_{i_1}^{\epsilon_1}...a_{i_{k-s}}^{\epsilon_{k-s}}a_{j_{s+1}}^{\tau_{s+1}}...a_{j_m}^{\tau_m}$ --- результат полного сокращения $W\tilde{W}$)\\
...
\begin{theorem}[Теорема Дика]
    $ \\$Пусть $G$ --- группа, заданная копредставлением $\langle A \ | \ \mathcal{R} \rangle$, где $A = \{a_i \ | \ i \in I\}$.\\ Пусть $H$ --- произвольная группа, $\{h_i \ | \ i \in I\} \subset H$ --- произвольное множество её элементов.\\
